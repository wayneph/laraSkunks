...
class DataLogic
{
    ...
    public static function enCryp(string $data)
    {
    ...
    private static function extendLongTokenLife(int $tokenId)
    {
    ...
    private static function getValidToken(int $tokenTypeId, int $userId)
    {
        $token=DB::connection('appControl')->table('tokens')
        ->select('id','token', 'token_type_id','type_use','user_id','status')
        ->where('user_id', $userId)
    ...
    public static function loginUser(string $userId, string $pin)
    {
        $user=DB::connection('appControl')->table('users')
            ->select('id', 'status')
            ->where('userid', $userId)
        ...
        $user=self::flattenArray($user);
        if($user['status']!=1){
            $user['err']=901;
            return $user;
        }
        $pinArray=self::getValidToken(2,$user['id']);
        if(!isset($pinArray[0])){
            $user['err']=903;
            return $user;
        }
        $pinArray=self::flattenArray($pinArray);
        if($pinArray['token']!=$pin){
            $user['err']=902;
            return $user;
        }
        self::setTokenExpiresById($pinArray['id'], env("USER_PIN_EXTEND_DAYS"));
        /* ALL OK SO FAR ---> SET SHORT & LONG TOKENS */
        $token=self::setUsageTokens($user['id']);
        $user['shortToken']=$token[0]['token'];
        return $user;
    }
    ...
    public static function setUser(array $inputs)
    {
        $retArray['status']=409;
        date_default_timezone_set ("Africa/Johannesburg");
